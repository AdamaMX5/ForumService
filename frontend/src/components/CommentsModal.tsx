      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3">
          ...
          <ul className="space-y-3">
            {items.map((comment) => (
              <li key={comment.id} className={comment.parent_comment_id ? 'ml-6 border-l-2 border-gray-200 pl-3 dark:border-gray-700' : ''}>
                ...
                <div className="mt-0.5 flex items-center gap-2 text-xs text-gray-400">
                  ...
                  {!comment.soft_deleted && (
                    <button type="button" onClick={() => setReplyTo(comment)} className="hover:text-blue-600">
                      Antworten
                    </button>
                  )}
                </div>
